Module pyflarum.extensions.flarum.FoF_UserBio
=============================================
...
`UserBioFlarumUserMixin(extensions: Optional[list] = None, **kwargs)`
:   The main object that carries the Flarum session.
    ...
    * builtins.dict
    ...
    `update_user_bio(self, bio: Optional[str] = None, user: Optional[ForwardRef('AnyUser')] = None) ‑> FlarumUser | User`
    :
...
`UserBioForumMixin(user: FlarumUser, _fetched_data: dict)`
...
`UserBioUserFromBulkMixin(user: FlarumUser, _fetched_data: dict)`
:   An user from `Users`.
